...
namespace Mvc3
{
    public class AccountController : Controller
    {
            ...
            string creditCardType, string creditCardNumber, string creditCardCVV2, string creditCardExpireMonth, string creditCardExpireYear)
        {
            ...
            string creditCardId = $"CARD-{Guid.NewGuid()}";
            string creditCardNumberPrefixMask = new string('x', creditCardNumber.Length - 4);
            string creditCardNumberSuffix = creditCardNumber.Substring(creditCardNumber.Length - 4, 4);
            string creditCardDescription = creditCardNumberPrefixMask + creditCardNumberSuffix;
            ...
            string newCreditCardType, string newCreditCardNumber, string newCreditCardCVV2, string newCreditCardExpireMonth, string newCreditCardExpireYear)
        {
            ...
            if (datTable != null && datTable.Rows.Count > 0)
            {
                ...
                string creditCardId = $"CARD-{Guid.NewGuid()}";
                string creditCardNumberPrefixMask = new string('x', newCreditCardNumber.Length - 4);
                string creditCardNumberSuffix = newCreditCardNumber.Substring(newCreditCardNumber.Length - 4, 4);
                string creditCardDescription = creditCardNumberPrefixMask + creditCardNumberSuffix;
        ...
        private bool DataBind(ProfileModel model, string email)
        {
            ...
            if (datTable != null && datTable.Rows.Count > 0)
            {
                ...
                string creditCardNumber = "4032032058954915";
                model.CurrentCreditCardNumber = creditCardNumber;
